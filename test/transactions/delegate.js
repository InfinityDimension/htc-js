...
describe('delegate.js', function () {
	...
	describe('#createDelegate', function () {
		...
		describe('returned delegate', function () {
			...
			var keys = htc.crypto.getKeys('secret');
			var secondKeys = htc.crypto.getKeys('secret 2');
			...
			it('should be signed correctly', function () {
				var result = htc.crypto.verify(trs, keys.publicKey);
				(result).should.be.ok;
			});

			it('should be second signed correctly', function () {
				var result = htc.crypto.verifySecondSignature(trs, secondKeys.publicKey);
				(result).should.be.ok;
			});
			...
			it('should not be second signed correctly now', function () {
				trs.amount = 100;
				var result = htc.crypto.verify(trs, secondKeys.publicKey);
				(result).should.be.not.ok;
			});

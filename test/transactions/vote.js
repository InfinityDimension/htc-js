...
describe('vote.js', function () {
	...
	describe('#createVote', function () {
		...
		var createVote = vote.createVote,
		    vt = null,
		    publicKey = htc.crypto.getKeys('secret').publicKey,
		...
		describe('returned vote', function () {
			...
			it('should have recipientId string equal to sender', function () {
				(vt).should.have.property('recipientId').and.be.type('string').and.equal(htc.crypto.getAddress(publicKey));
			});
			...
			it('should be signed correctly', function () {
				var result = htc.crypto.verify(vt);
				(result).should.be.ok;
			});

			it('should be second signed correctly', function () {
				var result = htc.crypto.verifySecondSignature(vt, htc.crypto.getKeys('second secret').publicKey);
				(result).should.be.ok;
			});

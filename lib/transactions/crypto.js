...
function getId(transaction) {
    var hash = crypto.createHash('sha256').update(getBytes(transaction).toString('hex'), 'hex').digest().toString('hex');
    return hash;
}
...
function getAddress(publicKey) {
    var sha256 = crypto.createHash('sha256').update(publicKey.toString('hex'), 'hex').digest();
    sha256 = crypto.createHash('ripemd160').update(sha256).digest();
    return bs58.encode(sha256) + 'L';
}
